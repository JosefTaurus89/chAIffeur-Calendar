...
export const AIFinancialAssistant: React.FC<AIFinancialAssistantProps> = ({ financialData }) => {
    ...
    // Hack to get language from local storage since props are not passed down in original structure
    // Ideally should be passed via props
    const getStoredLanguage = () => {
        try {
            const stored = window.localStorage.getItem('tour-management-settings-v5');
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 mt-8">
            <div className="flex items-center mb-4">
                ...
                <h3 className="text-xl font-semibold text-slate-800">{t('ai_financial_assistant')}</h3>
            </div>
            <p className="text-sm text-slate-500 mb-4">
                {t('ai_financial_assistant_desc')}
            </p>
            ...
            <div className="flex gap-2">
                ...
                >
                    {isLoading ? t('ai_analyzing') : t('get_insights')}
                </button>
            </div>
            ...
            {isLoading && (
                 <div className="mt-4 p-4 border border-slate-200 rounded-lg bg-slate-50 text-center">
                    ...
                    <p className="text-sm text-slate-600 mt-2">{t('ai_analyzing')}</p>
                 </div>
            )}
