...
export const SupplierReports: React.FC<SupplierReportsProps> = ({ suppliers, services, settings }) => {
  ...
  const supplierData = useMemo(() => {
    ...
    // Separate calculations for report columns
    const totalPassengerCash = outsourcedServices.reduce((sum, s) => {
        const isPrepaid = s.paymentMethod === 'Prepaid' || s.paymentMethod === 'Future Invoice';
        const price = s.clientPrice || 0;
        const deposit = s.deposit || 0;
        const balanceDue = isPrepaid ? 0 : Math.max(0, price - deposit);
    ...
    const totalExtras = outsourcedServices.reduce((sum, s) => sum + (s.extrasAmount || 0), 0);
  ...
  }, [selectedSupplierId, services, selectedMonth, searchQuery, suppliers, settings.language, settings.currency]);
  ...
  const handlePrint = () => {
    ...
    const metrics = [
        { label: t('period'), value: monthName },
        { label: t('amount_payable'), value: formatCurrency(supplierData.totalPayable) },
        { label: t('supplier_holds_cash'), value: formatCurrency(supplierData.totalHeldBySupplier) },
        { label: t('amount_receivable'), value: formatCurrency(supplierData.totalReceivable) },
        { label: t('net_balance'), value: `${supplierData.netBalance >= 0 ? '+' : ''}${formatCurrency(supplierData.netBalance)}` }
    ];
    ...
    const rows = supplierData.displayServices.map(s => {
        ...
        if (s.reportType === 'OUTSOURCED') {
             ...
             const isPrepaid = s.paymentMethod === 'Prepaid' || s.paymentMethod === 'Future Invoice';
             balance = isPrepaid ? 0 : Math.max(0, (s.clientPrice || 0) - (s.deposit || 0));
             extra = s.extrasAmount || 0;
             held = balance + extra;
             net = cost - held;
        } else {
            // Agency Job
        ...
        let heldDisplay = '-';
        if (s.reportType === 'OUTSOURCED') {
            // Only display the balance (passenger cash) in the "Supplier Holds" column
            if (balance > 0) {
                heldDisplay = `<b>${formatCurrency(balance)}</b>`;
            } else {
                heldDisplay = formatCurrency(0);
            }
        }

        let extrasDisplay = '-';
        if (s.extrasAmount || s.extrasInfo) {
            const amount = s.extrasAmount ? formatCurrency(s.extrasAmount) : '';
            const info = s.extrasInfo ? `<br/><span style="font-size:9px; color:#666;">${s.extrasInfo}</span>` : '';
            extrasDisplay = `${amount}${info}`;
        }
        ...
        return [
            ...
            s.title,
            s.paymentMethod || '-',
            s.reportType === 'OUTSOURCED' ? formatCurrency(cost) : '-',
            heldDisplay,
            s.reportType === 'AGENCY' ? formatCurrency(receivable) : '-',
            extrasDisplay, 
            `<span style="font-weight:bold; color:${net > 0 ? '#dc2626' : '#16a34a'}">${net > 0 ? 'To Prov: ' : 'To Agcy: '}${formatCurrency(Math.abs(net))}</span>`
        ]
    });
    ...
    rows.push([
        "", "", `<b>${t('totals_upper')}</b>`, 
        `<b>${formatCurrency(supplierData.totalPayable)}</b>`, 
        `<b>${formatCurrency(supplierData.totalPassengerCash)}</b>`, 
        `<b>${formatCurrency(supplierData.totalReceivable)}</b>`, 
        `<b>${formatCurrency(supplierData.totalExtras)}</b>`, 
        ""
    ]);
    ...
    rows.push([
        ...
        "", "", "", "",
        `<b style="font-size: 14px;">${formatCurrency(finalAmount)}</b><br/><i>${finalDirection}</i>`
    ]);

    printReport(
        `${t('statement_of_account')}: ${supplierData.supplierName}`, 
        `Generated via NCC`, 
        metrics, 
        headers, 
        rows
  ...
  const handleExportCSV = () => {
      ...
      const rows = supplierData.displayServices.map(s => {
          ...
          let balance = 0;
           ...
           let extrasStr = (s.extrasAmount || 0).toFixed(2);
           if (s.extrasInfo) extrasStr += ` (${s.extrasInfo})`;
          ...
          return [
            ...
            s.reportType === 'OUTSOURCED' ? balance.toFixed(2) : '0.00', // Export only balance for consistency with PDF
      ...
      rows.push(["", "", t('totals_upper'), 
          ...
          supplierData.totalPassengerCash.toFixed(2), 
          supplierData.totalReceivable.toFixed(2), 
          supplierData.totalExtras.toFixed(2), 
          ""
      ]);
            ...
            <div className="animate-fade-in-down space-y-8">
                                ...
                                {supplierData.yearlySummary.map((summary) => (
                                    <tr key={summary.month} className={`hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors ${summary.month === selectedMonth ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900 dark:text-slate-100">
                                            {new Date(summary.month + '-01').toLocaleString(getLocale(), { month: 'long', year: 'numeric' })}
                                        </td>
                ...
                <div className="flex flex-col md:flex-row justify-between items-end border-b border-slate-200 dark:border-slate-700 pb-4 mt-8">
                    <div>
                         <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">
                            {t('detailed_report')}: {new Date(selectedMonth + '-01').toLocaleString(getLocale(), { month: 'long', year: 'numeric' })}
                        </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <MetricCard 
                        ...
                        value={formatCurrency(supplierData.totalPayable)} 
                        subtitle={`${t('outstanding_balance')}: ${formatCurrency(supplierData.outstandingPayable)}`}
                        color="text-red-600 dark:text-red-400"
                    />
                    <MetricCard 
                        title={t('amount_receivable')} 
                        value={formatCurrency(supplierData.totalReceivable)} 
                        subtitle={`${t('outstanding_balance')}: ${formatCurrency(supplierData.outstandingReceivable)}`}
                        color="text-green-600 dark:text-green-400"
                    />
                    <MetricCard 
                        title={t('net_balance')} 
                        value={`${supplierData.netBalance >= 0 ? '+' : ''}${formatCurrency(supplierData.netBalance)}`} 
                    ...
                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        <thead className="bg-slate-50 dark:bg-slate-900">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-right text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase">{t('payable_cost')}</th>
                                <th className="px-6 py-3 text-right text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase">{t('receivable_price')}</th>
                                <th className="px-6 py-3 text-right text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase w-48">{t('extras')}</th>
                                <th className="px-6 py-3 text-right text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase">{t('net_balance')}</th>
                            </tr>
                        </thead>
                         <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                            {supplierData.displayServices.map(service => {
                                ...
                                if (isOutsourced) {
                                    ...
                                    const held = balance + (service.extrasAmount || 0);
                                    net = cost - held;
                                } else {
                                    net = -(service.clientPrice || 0);
                                }
                                ...
                                <tr key={service.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-red-600 dark:text-red-400 font-medium">
                                        {isOutsourced ? formatCurrency(service.supplierCost || 0) : '-'}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-green-600 dark:text-green-400 font-medium">
                                        {isOutsourced ? '-' : formatCurrency(service.clientPrice || 0)}
                                    </td>
                                    <td className="px-6 py-4 text-sm text-right text-purple-600 dark:text-purple-400 font-medium min-w-[150px]">
                                        <div>{service.extrasAmount ? formatCurrency(service.extrasAmount) : '-'}</div>
                                        {service.extrasInfo && <div className="text-xs text-slate-500 dark:text-slate-400 font-normal mt-0.5 whitespace-normal">{service.extrasInfo}</div>}
                                    </td>
                                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-right font-bold ${net > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
                                        {formatCurrency(Math.abs(net))}
                                        <span className="text-[10px] text-slate-400 font-normal ml-1">
                                            {net > 0 ? '(To Prov)' : '(To Agcy)'}
                                        </span>
                                    </td>
                                </tr>
                            )})}
