...
import { Service, ServiceStatus, User, Supplier, PaymentStatus, DriverAvailability, AppSettings, Vehicle } from '../../types';
...
export const ServiceDetailModal: React.FC<ServiceDetailModalProps> = ({ isOpen, onClose, service, onEdit, onDelete, drivers, suppliers, vehicles, isLoggedIn, settings, userRole }) => {
  ...
  // Internal drivers have no per-ride cost (salary)
  const supplierCost = supplier ? (service.supplierCost || 0) : 0;
  const extras = service.extrasAmount || 0;
  
  // Calculate Cash Held by Executor (including extras/commissions collected on site)
  const cashHeld = (() => {
      let amount = 0;
      const method = (service.paymentMethod || '').toLowerCase();
      
      // Logic: Cash, Card, Driver, Balance+Deposit -> Treated as collected on site
      // Credit Card is treated as collected by driver (POS/Terminal) unless explicitly Prepaid/Invoice
      if (
          ...
          if (!method.includes('prepaid') && !method.includes('invoice')) {
              amount = Math.max(0, clientPrice - deposit);
          }
      }
      
      // Add extras (commissions) as they are usually collected on site
      return amount + extras;
  })();

  const isPrepaidOrInvoice = (service.paymentMethod || '').toLowerCase().includes('prepaid') || (service.paymentMethod || '').toLowerCase().includes('invoice');
  ...
  if (supplier) {
      // Scenario: Outsourced
      // Net Payable = What we owe them (Cost) - What they took (Cash + Extras)
      const netPayableToSupplier = supplierCost - cashHeld;

      if (netPayableToSupplier > 0) {
          // We owe them
          netValue = netPayableToSupplier;
          netLabel = "Company owes Supplier"; 
          netColor = 'text-red-600 dark:text-red-400'; // Expense
          netExplanation = `(Cost $${supplierCost} > Held $${cashHeld})`;
      } else if (netPayableToSupplier < 0) {
          // They owe us
          netValue = Math.abs(netPayableToSupplier);
          netLabel = "Supplier owes Company";
          netColor = 'text-green-600 dark:text-green-400'; // Income
          netExplanation = `(Held $${cashHeld} > Cost $${supplierCost})`;
      } else {
          netValue = 0;
          netLabel = "Settled";
      ...
      // Scenario: Internal Driver (Salaried)
      // Profit isn't calculated per ride for driver payment.
      // They simply deposit what they collected.
      if (cashHeld > 0) {
          netValue = cashHeld;
          netLabel = "Driver Deposits to Office";
          netColor = 'text-orange-600 dark:text-orange-400';
          netExplanation = "Cash Collected";
      } else {
          netValue = 0;
          netLabel = "No Cash Handling";
          netColor = 'text-slate-500';
          netExplanation = "Prepaid/Invoice";
      }
  }
  ...
  const handleShare = async () => {
    ...
    if (service.agencyName) lines.push(`${t('agency')}: ${service.agencyName}`);
    
    // Pax breakdown
    const paxText = `${service.passengersAdults || 0} ${t('adults')}, ${service.passengersKids || 0} ${t('kids')}`;
    lines.push(`PAX: ${paxText}`);
    
    // Luggage breakdown if present
    if (service.passengersLuggageBig || service.passengersLuggageSmall) {
        lines.push(`LUGGAGE: ${service.passengersLuggageBig || 0} Big, ${service.passengersLuggageSmall || 0} Small`);
    }
    ...
    if (driver) {
        ...
        if (vehicle) lines.push(`Vehicle: ${vehicle.make} ${vehicle.model} (${vehicle.plate})`);
    } else if (supplier) {
  ...
  const handleDelete = async () => {
    if (window.confirm(t('confirm_delete_service'))) {
        ...
        onDelete(service.id);
        alert(t('service_moved_to_trash'));
        onClose();
    }
  }
  ...
  const handleSaveAssignment = () => {
      ...
      if (assignType === 'driver') {
          ...
          updatedService.vehicleId = undefined; // Clear vehicle if supplier assigned
      }
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex justify-center items-center p-4 overflow-hidden">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[95vh] animate-fade-in-up">
                 ...
                 <div className="flex items-center gap-2 mb-1">
                    ...
                    {service.googleCalendarEventId && (
                        <div className="flex items-center text-xs text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-700 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-600">
                            <svg className="w-3 h-3 mr-1" viewBox="0 0 24 24" fill="currentColor"><path d="M21 6.5c0-.28-.22-.5-.5-.5h-2v-1.5c0-.28-.22-.5-.5-.5h-2c-.28 0-.5.22-.5.5v1.5h-5V4.5c0-.28-.22-.5-.5-.5h-2c-.28 0-.5.22-.5.5v1.5h-2c-.28 0-.5.22-.5.5v1.5h-2c-.28 0-.5.22-.5.5v14c0 .28.22.5.5.5h17c.28 0 .5-.22.5-.5v-14zM11.5 11H6V9h5.5v2zm-1 2.5H6v2h4.5v-2zm.5 3.5h-5V19h5v-2zm6-5.5h-4V9h4v2.5z" /></svg>
                            Synced
                        </div>
                    )}
                </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            {/* Agency Banner if applicable */}
            {service.agencyName && (
                <div className="mb-6 p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-lg flex items-center">
                    <div className="p-2 bg-indigo-100 dark:bg-indigo-800 rounded-full mr-3 text-indigo-600 dark:text-indigo-200">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                    </div>
                    <div>
                        <p className="text-xs font-bold text-indigo-500 uppercase tracking-wide">{t('booked_by_agency')}</p>
                        <p className="text-lg font-bold text-indigo-900 dark:text-indigo-100">{service.agencyName}</p>
                    </div>
                </div>
            )}
            ...
            <div className="mb-6">
            {isAssigning ? (
                <div className="p-4 border-2 border-primary-500 rounded-lg bg-primary-50 dark:bg-slate-700 animate-fade-in-down">
                    ...
                    <div className="space-y-3">
                         ...
                         <select 
                            ...
                            className="block w-full bg-slate-50 text-slate-900 border-slate-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500 sm:text-sm dark:bg-slate-800 dark:border-slate-600 dark:text-slate-200 py-2.5"
                        >
                    ...
                    <div className="flex items-center">
                        ...
                        <div>
                            ...
                            <p className="font-semibold text-slate-900 dark:text-white">
                                {driver ? (
                                    <span>{driver.name} {vehicle && <span className="text-xs font-normal text-slate-500">({vehicle.make} {vehicle.model})</span>}</span>
                                ) : supplier ? supplier.name : t('unassigned')}
                            </p>
                        </div>
                    </div>
                    {isAdmin && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
                ...
                {(service.passengersLuggageBig || service.passengersLuggageSmall) && (
                    <DetailItem 
                        label={t('luggage')} 
                        value={
                            <span className="flex items-center gap-3">
                                {service.passengersLuggageBig ? <span>{service.passengersLuggageBig} {t('luggage_big')}</span> : null}
                                {service.passengersLuggageBig && service.passengersLuggageSmall ? <span className="text-slate-500">|</span> : null}
                                {service.passengersLuggageSmall ? <span>{service.passengersLuggageSmall} {t('luggage_small')}</span> : null}
                            </span>
                        }
                        icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>}
                    />
                )}
                    ...
                    <div className="col-span-1 sm:col-span-2 mt-2 pt-2 border-t border-slate-100 dark:border-slate-700">
                         ...
                         <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            <div className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded border border-slate-100 dark:border-slate-600">
                                <p className="text-[10px] text-slate-500 uppercase">{t('client_price')} / {t('profit')}</p>
                            ...
                            {supplier && (
                                 <>
                                    ...
                                    <div className="bg-slate-50 dark:bg-slate-700/50 p-2 rounded border border-slate-100 dark:border-slate-600 col-span-2">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <p className="text-[10px] text-slate-400 italic text-right max-w-[150px]">{netExplanation}</p>
                                        </div>
                                    </div>
                                 </>
                            )}
                ...
                {service.description && (
                     <div className="col-span-1 sm:col-span-2 mt-4">
                         <div className="flex items-center mb-1">
                            ...
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">{t('original_description')}</p>
                         </div>
                         <div className="bg-slate-50 dark:bg-slate-700/20 p-3 rounded-md border border-slate-200 dark:border-slate-600 text-xs font-mono text-slate-600 dark:text-slate-400 whitespace-pre-wrap max-h-32 overflow-y-auto">
                             {service.description}
                         </div>
                     </div>
                )}
            ...
            {isAdmin && (
                <div className="flex space-x-3 w-full sm:w-auto">
                    <button
                        ...
                        className="flex-1 sm:flex-none flex items-center justify-center px-4 py-2 text-sm font-bold text-white bg-red-600 rounded-lg hover:bg-red-700 shadow-sm transition-colors"
                        title={t('delete')}
                    >
