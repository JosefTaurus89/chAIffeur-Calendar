...
export const DriverReports: React.FC<DriverReportsProps> = ({ drivers, services }) => {
  ...
  const driverData = useMemo(() => {
    ...
    // Logic: If paymentMethod is 'Cash', 'Card', 'Driver' related, or 'Balance' related, driver holds amount.
    const cashCollected = driverServices.reduce((sum, s) => {
        ...
        if (
            ...
            if (!method.includes('prepaid') && !method.includes('invoice')) {
                amount = Math.max(0, price - deposit);
            }
        }
                            ...
                            {driverData.filteredServices.map(service => {
                                ...
                                <tr key={service.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
                                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${isCollected ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300 ring-1 ring-green-500' : 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'}`}>
