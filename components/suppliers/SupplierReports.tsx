...
export const SupplierReports: React.FC<SupplierReportsProps> = ({ suppliers, services, settings }) => {
  ...
  const handlePrint = () => {
    ...
    const rows = supplierData.displayServices.map(s => {
        ...
        if (s.reportType === 'OUTSOURCED') {
            if (held > 0) {
                // Clearly distinguish extra from base cash
                heldDisplay = `<b>${formatCurrency(held)}</b>`;
                const breakdownParts = [];
                if (balance > 0) breakdownParts.push(`${t('base')}: ${formatCurrency(balance)}`);
                if (extra > 0) breakdownParts.push(`${t('incl_extra')}: ${formatCurrency(extra)}${s.extrasInfo ? ` (${s.extrasInfo})` : ''}`);
                
                if (breakdownParts.length > 0) {
                    heldDisplay += `<br/><span style="font-size:9px; color:#64748b; font-style:italic;">(${breakdownParts.join(' + ')})</span>`;
                }
            } else {
        ...
        return [
            ...
            s.title,
            s.reportType === 'OUTSOURCED' ? t('outsourced') : t('agency_job'),
            s.paymentMethod || '-',
    ...
    rows.push([
        ...
        `<b>${formatCurrency(supplierData.totalHeldBySupplier)}</b>`, 
                                ...
                                <tr key={service.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${isOutsourced ? 'bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-300' : 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300'}`}>
                                            {isOutsourced ? t('outsourced') : t('agency_job')}
                                        </span>
                                    </td>
